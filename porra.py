...
def fetch_fixtures_liga(liga_id, season):
    """Carga últimos 5 + próximos 10 partidos de una liga."""
    partidos = []
    for param, n in [("last", 5), ("next", 10)]:
        try:
            r = requests.get(
                f"{BASE_URL}/fixtures",
                headers=HEADERS_API,
                params={"league": liga_id, "season": season, param: n, "timezone": "Europe/Madrid"},
                timeout=15
            )
            data = r.json()
            for f in data.get("response", []):
                ...
                try:
                    ...
                    fecha_fmt = dt.strftime("%a %d %b · %H:%M")
                except:
                    fecha_fmt = fixture["date"][:10]
...
def get_partidos():
    ...
    for comp, info in LIGAS.items():
        result[comp] = fetch_fixtures_liga(info["id"], info["season"])
    return result
...
def registrar(nombre, password):
    r = requests.get(f"{SUPABASE_URL}/rest/v1/usuarios?nombre=eq.{nombre}&select=id", headers=HEADERS_SB, timeout=10)
    if r.ok and r.json():
...
def unirse_grupo(codigo, user_id):
    r = requests.get(f"{SUPABASE_URL}/rest/v1/grupos?codigo=eq.{codigo}&select=*", headers=HEADERS_SB, timeout=10)
...
def get_mis_grupos(user_id):
    ...
    r2 = requests.get(f"{SUPABASE_URL}/rest/v1/grupos?id=in.({ids_str})&select=*", headers=HEADERS_SB, timeout=10)
...
def eliminar_apuesta(apuesta_id):
    requests.delete(f"{SUPABASE_URL}/rest/v1/apuestas?id=eq.{apuesta_id}", headers=HEADERS_SB, timeout=10)
...
with tab_partidos:
    ...
    if not partidos_comp:
        st.warning("No hay partidos disponibles. La lista se actualiza cada 24h.")
    else:
        opciones = []
...
with tab_apuestas_tab:
    if not apuestas:
        ...
        for bet in lista_filtrada:
            ...
            partido_str    = f"{partido['home']} vs {partido['away']}" if partido else bet.get("partido_id", "?")
